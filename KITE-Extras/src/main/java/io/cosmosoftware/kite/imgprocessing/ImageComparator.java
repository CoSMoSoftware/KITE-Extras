...
public class ImageComparator {
  ...
  public static List<Boolean> compareElements(
    ...
    return compareRectangles(getRectangleList(mobileElementList), offset, createImageFromBytes(screenShot1Bytes),
  ...
  public static List<Boolean> compareRectanglesWithExpectedChecksum(
    ...
    List<BufferedImage> bufferedImageList =
      ImageComparator.getBufferedImageList(createImageFromBytes(screenShot), windowSize, rectangleList, offset);
    ...
    for (BufferedImage bufferedImage : bufferedImageList) {
      ...
      try {
        ...
        throw new KiteTestException(e.getClass().getName() + " comparing rectangle with checksum", Status.BROKEN, e);
      }
    }
  ...
  public static BufferedImage createImageFromBytes(byte[] imageData) throws KiteTestException {
    try {
      ...
      throw new KiteTestException(e.getClass().getName() + " create Image from Bytes", Status.BROKEN, e);
    }
  }
   ...
   * @throws KiteTestException the kite test exception
   */
  public static int getPixelSumDiff(BufferedImage image1, BufferedImage image2) throws KiteTestException {
    
    List<Integer> image1Pixels = getNormalizedPixelValues(image1);
    List<Integer> image2Pixels = getNormalizedPixelValues(image2);
    
    int finalSize = image1Pixels.size() > image2Pixels.size() ? image1Pixels.size() : image2Pixels.size();
    ...
    for (int index : array) {
      if (res > Integer.MAX_VALUE - index) {
        throw new KiteTestException("The sum is too large at index " +
          "[" + index + "/" + array.size() + "], try again with smaller array/photo.", Status.BROKEN);
      }
      res += index;
    }
    return res;
  ...
  private static void writeImage(BufferedImage bufferedImage, String formatName, File output)
    ...
    if (debug) {
      try {
        ...
        throw new KiteTestException(e.getClass().getName() + " writing Image for debug purpose", Status.BROKEN, e, true);
      }
    }
  }
}
