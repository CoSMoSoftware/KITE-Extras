...
public class VideoRecorder {
  ...
  private static Format getDefaultMouseFormat() {
    return new Format(MediaTypeKey, MediaType.VIDEO, EncodingKey,
   ...
   */
  public void startRecording(Point startingPoint, int width, int height, String videoName)
    ...
    try {
      this.recorder = new CustomizedScreenRecorder(
          ...
          startingPoint == null
              ...
              : new Rectangle((int) startingPoint.getX(), (int) startingPoint.getY(), width,
                  height),
      ...
      throw new KiteTestException(
          "Error while creating video recorder and recording video: " + e.getLocalizedMessage(),
          Status.BROKEN);
    }
  }
  ...
  /**
   * Start recording the area of a web element, relatively to the location and size of the
   * webdriver's windows
   *
   ...
   */
  public void startRecording(WebElement element, WebDriver webDriver, String videoName)
      throws Exception {
    ...
    Point startingPoint = new Point(windowLocation.getX() + elementLocation.getX(),
        windowLocation.getY() + elementLocation.getY() + topBarMargin);
    startRecording(startingPoint, elementDimension.getWidth(), elementDimension.getHeight(),
        videoName);
  }
  ...
  public void stopRecording() throws KiteTestException {
    if (this.recorder != null) {
      try {
        ...
        throw new KiteTestException(
            "Error while stopping recording video: " + e.getLocalizedMessage(), Status.BROKEN);
      }
    }
  }
